	...
	JAFO.addFileOpenTab = function() {
		...
		tab.innerHTML =
			'<a id=tabFileOpen title="Select a file to load or start a new file" ><p class=buttonFile >' +
				'<i class="fa fa-plus-square-o"></i> Load File...' +
			'</p></a>';
		...
		JAFO.FileOpenTab.innerHTML =
			'<p title="Adjust size of incoming object" >Scale Model: <input type=number id=inpScale value=1.000 max=1000 min=0.001 step=1 /></p>' +
			'<p>Open New .obj Unity Model: <input type=file id=inpOpenFile ></p>' +
			'<p title="Click to Load a JS format model" >Open New .json Binary Model: <input type=file id=inpOpenBinaryFile ></p>' +
			'<p>Open New .ifc BIM Model: <input type=file id=inpAppendFile ></p>' +
			'<p>Notes: opening a .ifc BIM model is currently under development' +
			'</p>' +
		'';
		// BU KISIM YENI DOSYA FORMATLARINA GORE DEGISECEK
